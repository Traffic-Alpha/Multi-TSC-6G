...
@LastEditTime: 2024-05-06 21:33:24
'''
import time
import numpy as np
...
from ._utils import TimeSeriesData, direction_to_flags, merge_local_data, one_hot_encode
...
class GlobalLocalInfoWrapper(gym.Wrapper):
    ...
    def get_vehicle_obs(self, vehicle_data:Dict[str, Dict[str, Any]]):
        ...
        for _veh_id, _veh_info in vehicle_data.items():
            _lane_id = _veh_info['lane_id']
            if not _lane_id.startswith(":"):
                _speed = _veh_info['speed']
                _lane_position = _veh_info['lane_position']/self.lane_infos[_lane_id]['length']
                _waiting_time = _veh_info['waiting_time']
                _accumulated_waiting_time = _veh_info['accumulated_waiting_time']
                _edge_id = one_hot_encode(self.road_ids, _veh_info['road_id'])
                processed_veh_obs[_veh_id] = [_speed, _lane_position, _waiting_time, _accumulated_waiting_time] + _edge_id
        return processed_veh_obs
    ...
    def process_veh_state(self, K=1):
        """获得最后一个时刻车辆的信息

        Args:
            K (int, optional): 去 K 个时间片, 这里车辆的信息只使用最后一个时刻的信息. Defaults to 1.
        """
        _recent_k_data = self.vehicle_timeseries.get_recent_k_data(K)[-1][-1]
        # Concatenate the values
        result = [value for value in _recent_k_data.values()]
        # If the number of lists is less than 100, pad with lists of zeros
        if len(result) < 100:
            result += [[0]*len(result[0]) for _ in range(100 - len(result))]
        # If the number of lists is more than 100, take the first 100 lists
        else:
            result = result[:100]
    ...
    def reset(self, seed=1):
        ...
        self.road_ids = sorted(set([_lane['edge_id'] for _,_lane in self.lane_infos.items()])) # 获得所有的 edge id
        ...
        processed_veh_obs = np.random.randn(100,24)
